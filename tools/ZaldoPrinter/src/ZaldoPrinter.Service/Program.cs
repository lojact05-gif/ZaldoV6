...
app.MapPost("/print/fiscal-pdf", async (HttpContext context, FiscalPdfPrintRequest request, JsonConfigStore store, EscPosBuilder escPos, PrinterQueueDispatcher dispatcher, FileLog logger) =>
{
    var config = store.Get();
    var profile = ResolveTargetPrinter(context, config, null);
    ...
    if (pages.Count == 0)
    {
        return Results.UnprocessableEntity(new { ok = false, error = "No fiscal pages provided.", printerId = profile.Id });
    }

    if (pages.Count > 20)
    {
        return Results.UnprocessableEntity(new { ok = false, error = "Too many fiscal pages.", printerId = profile.Id });
    }
    ...
    {
        var applyCut = request.Cut ?? profile.Cut.Enabled;
        var applyDrawer = request.OpenDrawer ?? false;
        bytes = escPos.BuildFiscalPdfPages(
            pages,
            profile,
            applyCut,
            request.CutMode,
    ...
    {
        logger.Error($"Fiscal PDF build failed on printer '{profile.Id}'.", ex);
        return Results.UnprocessableEntity(new { ok = false, error = "Failed to prepare fiscal pages for printing.", printerId = profile.Id });
    }

    var result = await EnqueueAndWait(dispatcher, profile, bytes, config, PrintOperationType.Raw, context.RequestAborted);
    if (!result.Ok)
    {
        logger.Error($"Fiscal PDF print failed on printer '{profile.Id}': {result.Message}");
        return Results.UnprocessableEntity(new { ok = false, error = result.Message, printerId = profile.Id, jobId = result.JobId });
    }

    return Results.Ok(new { ok = true, message = "Fiscal PDF printed.", printerId = profile.Id, jobId = result.JobId, attempts = result.Attempts });
});
